...
// CDC devices often implement Interface Association Descriptor (IAD). Parse IAD
// only when bDeviceClass = 0xEF (Miscellaneous Device Class), bDeviceSubClass =
// 0x02 (Common Class), bDeviceProtocol = 0x01 (Interface Association
// Descriptor), or when bDeviceClass, bDeviceSubClass, and bDeviceProtocol are
// 0x00 (Null class code triple), as per
// https://www.usb.org/defined-class-codes, "Base Class 00h (Device)" section
// @see USB Interface Association Descriptor: Device Class Code and Use Model
// rev 1.0, Table 1-1
...
typedef struct {
  usb_host_client_handle_t
      cdc_acm_client_hdl; /*!< USB Host handle reused for all CDC-ACM devices in
                             the system */
 ...
 *
 * This configuration is used when user passes NULL to config pointer during
 * device open.
 */
...
typedef struct {
  ...
  union {
    struct {
      ...
      uint8_t call_over_data_if : 1;  // Device sends/receives call management
                                      // information over Data Class interface
      uint8_t reserved : 6;
    };
    uint8_t val;
  } bmCapabilities;
  uint8_t bDataInterface;  // Interface number of Data Class interface
...
typedef struct {
  ...
  union {
    struct {
      uint8_t
          feature : 1;  // Device supports Set/Clear/Get_Comm_Feature requests
      uint8_t serial : 1;  // Device supports Set/Get_Line_Coding,
                           // Set_Control_Line_State and Serial_State request
                           // and notifications
...
struct cdc_dev_s {
  ...
  void *cb_arg;  // Common argument for user's callbacks (data IN and
                 // Notification)
  struct {
    usb_transfer_t *out_xfer;  // OUT data transfer
    usb_transfer_t *in_xfer;   // IN data transfer
    cdc_acm_data_callback_t
        in_cb;        // User's callback for async (non-blocking) data IN
    uint16_t in_mps;  // IN endpoint Maximum Packet Size
    uint8_t
        *in_data_buffer_base;  // Pointer to IN data buffer in usb_transfer_t
  ...
  struct {
    ...
    const usb_intf_desc_t
        *intf_desc;  // Pointer to notification interface descriptor, can be
                     // NULL if there is no notification channel in the device
  ...
  int num_cdc_intf_desc;  // Number of CDC Interface descriptors in following
                          // array
 ...
 *
 * Notification (interrupt) IN transfer is submitted at the end of this function
 * to ensure periodic poll of IN endpoint.
 *
 ...
 *
 * Data (bulk) IN transfer is submitted at the end of this function to ensure
 * continuous poll of IN endpoint.
 *
 * @param[in] transfer Transfer that triggered the callback
 */
 ...
 * Both IN and OUT requests are sent through this API, depending on the
 * in_transfer parameter.
 *
 * @see  Chapter 6.2, USB CDC specification rev. 1.2
 * @note CDC specific requests are only supported by devices that have dedicated
 * management element.
 *
 ...
 * @param[inout] data Pointer to data buffer. Input for OUT transfers, output
 * for IN transfers.
 * @param[in] data_len Length of data buffer
 * @param[in] value Value to be set in bValue of Setup packet
 * @return esp_err_t
 */
static esp_err_t send_cdc_request(cdc_dev_t *cdc_dev, bool in_transfer,
                                  cdc_request_code_t request, uint8_t *data,
                                  uint16_t data_len, uint16_t value);

/**
 * @brief Reset IN transfer
 *
 * In in_xfer_cb() we can modify IN transfer parameters, this function resets
 * the transfer to its defaults
 *
 * @param[in] cdc_dev Pointer to CDC device
 */
...
static void cdc_acm_client_task(void *arg) {
  ...
  cdc_acm_obj_t *cdc_acm_obj =
      p_cdc_acm_obj;  // Make local copy of the driver's handle
 ...
 *
 * This function will cancel ongoing transfer a reset its endpoint to ready
 * state.
 *
 ...
 * @return esp_err_t
 */
static esp_err_t cdc_acm_reset_transfer_endpoint(usb_device_handle_t dev_hdl,
                                                 usb_transfer_t *transfer) {
  assert(dev_hdl);
  assert(transfer);

  ESP_RETURN_ON_ERROR(
      usb_host_endpoint_halt(dev_hdl, transfer->bEndpointAddress), TAG, );
  ESP_RETURN_ON_ERROR(
      usb_host_endpoint_flush(dev_hdl, transfer->bEndpointAddress), TAG, );
 ...
 * @param[in] user_arg  Optional user's argument, that will be passed to the
 * callbacks
 * @return esp_err_t
 */
static esp_err_t cdc_acm_start(cdc_dev_t *cdc_dev,
                               cdc_acm_host_dev_callback_t event_cb,
                               cdc_acm_data_callback_t in_cb, void *user_arg) {
  ...
  ESP_GOTO_ON_ERROR(usb_host_interface_claim(
                        p_cdc_acm_obj->cdc_acm_client_hdl, cdc_dev->dev_hdl,
                        cdc_dev->data.intf_desc->bInterfaceNumber, 0),
                    err, TAG, );
  ...
  // If notification are supported, claim its interface and start polling its IN
  // endpoint
  if (cdc_dev->notif.xfer) {
    if (cdc_dev->notif.intf_desc != cdc_dev->data.intf_desc) {
      ESP_GOTO_ON_ERROR(usb_host_interface_claim(
                            p_cdc_acm_obj->cdc_acm_client_hdl, cdc_dev->dev_hdl,
                            cdc_dev->notif.intf_desc->bInterfaceNumber, 0),
                        err, TAG, );
    }
...
err:
  usb_host_interface_release(p_cdc_acm_obj->cdc_acm_client_hdl,
                             cdc_dev->dev_hdl,
 ...
 *
 * @note All interfaces claimed by this device must be release before calling
 * this function
...
static void cdc_acm_device_remove(cdc_dev_t *cdc_dev) {
  ...
  // We don't check the error code of usb_host_device_close, as the close might
  // fail, if someone else is still using the device (not all interfaces are
  // released)
  usb_host_device_close(p_cdc_acm_obj->cdc_acm_client_hdl,
                        cdc_dev->dev_hdl);  // Gracefully continue on error
  free(cdc_dev);
}
 ...
 * 1. USB device with matching VID/PID is already opened by this driver:
 * allocate new CDC device on top of the already opened USB device.
 * 2. USB device with matching VID/PID is NOT opened by this driver yet: poll
 * USB connected devices until it is found.
 *
 * @note This function will block for timeout_ms, if the device is not
 * enumerated at the moment of calling this function.
 ...
 * @return esp_err_t
 */
static esp_err_t cdc_acm_find_and_open_usb_device(uint16_t vid, uint16_t pid,
                                                  int timeout_ms,
                                                  cdc_dev_t **dev) {
  ...
  *dev = calloc(1, sizeof(cdc_dev_t));
  ...
  SLIST_FOREACH(cdc_dev, &p_cdc_acm_obj->cdc_devices_list, list_entry) {
    ...
    ESP_ERROR_CHECK(
        usb_host_get_device_descriptor(cdc_dev->dev_hdl, &device_desc));
  ...
  do {
    ...
    int num_of_devices;
    ESP_ERROR_CHECK(usb_host_device_addr_list_fill(
    ...
    for (int i = 0; i < num_of_devices; i++) {
      ...
      // Open USB device
      if (usb_host_device_open(p_cdc_acm_obj->cdc_acm_client_hdl,
                               dev_addr_list[i], &current_device) != ESP_OK) {
        continue;  // In case we failed to open this device, continue with next
                   // one in the list
      }
  ...
  } while (xTaskCheckForTimeOut(&connection_timeout, &timeout_ticks) ==
           pdFALSE);
...
esp_err_t cdc_acm_host_install(
  ...
  xTaskCreatePinnedToCore(cdc_acm_client_task, "USB-CDC",
                          driver_config->driver_task_stack_size, NULL,
                          driver_config->driver_task_priority, &driver_task_h,
                          driver_config->xCoreID);
...
esp_err_t cdc_acm_host_uninstall() {
  ...
  cdc_acm_obj_t *cdc_acm_obj =
      p_cdc_acm_obj;  // Save Driver's handle to temporary handle
  CDC_ACM_EXIT_CRITICAL();

  xSemaphoreTake(p_cdc_acm_obj->open_close_mutex,
                 portMAX_DELAY);  // Wait for all open/close calls to finish

  CDC_ACM_ENTER_CRITICAL();
  if (SLIST_EMPTY(
          &p_cdc_acm_obj->cdc_devices_list)) {  // Check that device list is
                                                // empty (all devices closed)
    p_cdc_acm_obj = NULL;  // NULL static driver pointer: No open/close calls
                           // form this point
  } else {
    ret = ESP_ERR_INVALID_STATE;
    CDC_ACM_EXIT_CRITICAL();
  ...
  ESP_GOTO_ON_FALSE(
      xEventGroupWaitBits(cdc_acm_obj->event_group, CDC_ACM_TEARDOWN_COMPLETE,
...
esp_err_t cdc_acm_host_register_new_dev_callback(
    cdc_acm_new_dev_callback_t new_dev_cb) {
  CDC_ACM_ENTER_CRITICAL();
 ...
 *
 * @note There can be no transfers in flight, at the moment of calling this
 * function.
 * @param[in] cdc_dev Pointer to CDC device
 */
static void cdc_acm_transfers_free(cdc_dev_t *cdc_dev) {
 ...
 * @return
 *     - ESP_OK:            Success
 *     - ESP_ERR_NO_MEM:    Not enough memory for transfers and semaphores
 * allocation
 *     - ESP_ERR_NOT_FOUND: IN or OUT endpoints were not found in the selected
 * interface
 */
static esp_err_t cdc_acm_transfers_allocate(cdc_dev_t *cdc_dev,
                                            const usb_ep_desc_t *notif_ep_desc,
  ...
  // If your open functions fail here, it signals that you either selected wrong
  // interface number or that you are trying to open IAD CDC device with
  // cdc_acm_host_open_vendor_specific() instead of cdc_acm_host_open() Refer to
  // README.md for more information
  ESP_RETURN_ON_FALSE(in_ep_desc && out_ep_desc, ESP_ERR_NOT_FOUND, TAG,
                      "IN or OUT endpoint not found in this data interface");

  // 1. Setup notification transfer if it is supported
  if (notif_ep_desc) {
    ESP_GOTO_ON_ERROR(
        usb_host_transfer_alloc(USB_EP_DESC_GET_MPS(notif_ep_desc), 0,
                                &cdc_dev->notif.xfer),
        err, TAG, );
  ...
  ESP_GOTO_ON_ERROR(usb_host_transfer_alloc(dev_info.bMaxPacketSize0, 0,
  ...
  ESP_GOTO_ON_FALSE(cdc_dev->ctrl_transfer->context, ESP_ERR_NO_MEM, err,
                    TAG, );
  ...
  if (in_buf_len != 0) {
    ESP_GOTO_ON_ERROR(
        usb_host_transfer_alloc(in_buf_len, 0, &cdc_dev->data.in_xfer), err,
        TAG, );
    assert(cdc_dev->data.in_xfer);
  ...
  if (out_buf_len != 0) {
    ESP_GOTO_ON_ERROR(
        usb_host_transfer_alloc(out_buf_len, 0, &cdc_dev->data.out_xfer), err,
        TAG, );
    ...
    ESP_GOTO_ON_FALSE(cdc_dev->data.out_xfer->context, ESP_ERR_NO_MEM, err,
                      TAG, );
 ...
 *
 * @note This function is called in open procedure of CDC compliant devices
 * only.
 * @param[in]  cdc_dev  Pointer to CDC device
 * @param[in]  intf_idx Index of CDC interface that should be used for this
 * device
 ...
 */
static esp_err_t cdc_acm_find_intf_and_ep_desc(cdc_dev_t *cdc_dev,
                                               uint8_t intf_idx,
  ...
  if (((device_desc->bDeviceClass == USB_CLASS_MISC) &&
    ...
    do {
      ...
      if (iad_desc->bFirstInterface == intf_idx) {
        // IAD with correct interface number was found: Check Class/Subclass
        // codes, save Interface indexes
  ...
  if (interface_found) {
    ...
    cdc_dev->notif.intf_desc = usb_parse_interface_descriptor(
        config_desc, notif_intf_idx, 0, &desc_offset);
    assert(cdc_dev->notif.intf_desc);

    // CDC specific descriptors should be right after CDC-Communication
    // interface descriptor Note: That's why we use usb_parse_next_descriptor
    // instead of usb_parse_next_descriptor_of_type. The latter could return CDC
    // specific descriptors that don't belong to this interface
    const usb_standard_desc_t *cdc_desc =
        (usb_standard_desc_t *)cdc_dev->notif.intf_desc;
    do {
      cdc_desc = usb_parse_next_descriptor(cdc_desc, config_desc->wTotalLength,
                                           &desc_offset);
      if ((cdc_desc == NULL) ||
          (cdc_desc->bDescriptorType !=
           ((USB_CLASS_COMM << 4) | USB_B_DESCRIPTOR_TYPE_INTERFACE))) {
    ...
    *notif_ep = usb_parse_endpoint_descriptor_by_index(
        cdc_dev->notif.intf_desc, 0, config_desc->wTotalLength, &desc_offset);
    assert(notif_ep);

    // Data IF and EP
    cdc_dev->data.intf_desc = usb_parse_interface_descriptor(
        config_desc, data_intf_idx, 0, &desc_offset);
...
esp_err_t cdc_acm_host_open_vendor_specific(
...
esp_err_t cdc_acm_host_close(cdc_acm_dev_hdl_t cdc_hdl) {
  ...
  CDC_ACM_EXIT_CRITICAL();
  if (cdc_dev->data.in_xfer) {
    ESP_ERROR_CHECK(cdc_acm_reset_transfer_endpoint(cdc_dev->dev_hdl,
                                                    cdc_dev->data.in_xfer));
  }
  if (cdc_dev->notif.xfer != NULL) {
    ESP_ERROR_CHECK(
        cdc_acm_reset_transfer_endpoint(cdc_dev->dev_hdl, cdc_dev->notif.xfer));
  }
  ...
  CDC_ACM_ENTER_CRITICAL();
  SLIST_REMOVE(&p_cdc_acm_obj->cdc_devices_list, cdc_dev, cdc_dev_s,
               list_entry);
  CDC_ACM_EXIT_CRITICAL();
...
static void cdc_acm_print_desc(const usb_standard_desc_t *_desc) {
  if (_desc->bDescriptorType !=
      ((USB_CLASS_COMM << 4) | USB_B_DESCRIPTOR_TYPE_INTERFACE)) {
    // Quietly return in case that this descriptor is not CDC interface
    // descriptor
    return;
  }
...
static bool cdc_acm_is_transfer_completed(usb_transfer_t *transfer) {
  ...
  switch (transfer->status) {
    case USB_TRANSFER_STATUS_COMPLETED:
      completed = true;
      break;
    case USB_TRANSFER_STATUS_NO_DEVICE:  // User is notified about device
                                         // disconnection from usb_event_cb
...
static void in_xfer_cb(usb_transfer_t *transfer) {
  ...
  if (cdc_dev->data.in_cb) {
    const bool data_processed = cdc_dev->data.in_cb(
        transfer->data_buffer, transfer->actual_num_bytes, cdc_dev->cb_arg);

    // Information for developers:
    // In order to save RAM and CPU time, the application can indicate that the
    // received data was not processed and that the application expects more
    // data. In this case, the next received data must be appended to the
    // existing buffer. Since the data_buffer in usb_transfer_t is a constant
    // pointer, we must cast away to const qualifier.
    if (!data_processed) {
      // In case the received data was not processed, the next RX data must be
      // appended to current buffer
      uint8_t **ptr = (uint8_t **)(&(transfer->data_buffer));
      *ptr += transfer->actual_num_bytes;

      // Calculate remaining space in the buffer. Attention: pointer
      // arithmetics!
      size_t space_left =
          transfer->data_buffer_size -
          (transfer->data_buffer - cdc_dev->data.in_data_buffer_base);
      uint16_t mps = cdc_dev->data.in_mps;
      transfer->num_bytes =
          (space_left / mps) * mps;  // Round down to MPS for next transfer

      if (transfer->num_bytes == 0) {
        // The IN buffer cannot accept more data, inform the user and reset the
        // buffer
...
static void notif_xfer_cb(usb_transfer_t *transfer) {
  ...
  if (cdc_acm_is_transfer_completed(transfer)) {
    ...
    switch (notif->bNotificationCode) {
      ...
      case USB_CDC_NOTIF_RESPONSE_AVAILABLE:  // Encapsulated commands not
                                              // implemented - fallthrough
      default:
        ESP_LOGW("CDC_ACM", "Unsupported notification type 0x%02X",
                 notif->bNotificationCode);
        ESP_LOG_BUFFER_HEX("CDC_ACM", transfer->data_buffer,
...
static void usb_event_cb(const usb_host_client_event_msg_t *event_msg,
  ...
  switch (event_msg->event) {
    ...
    case USB_HOST_CLIENT_EVENT_DEV_GONE: {
      ...
      cdc_dev_t *cdc_dev;
      cdc_dev_t *tcdc_dev;
      // We are using 'SAFE' version of 'SLIST_FOREACH' which enables user to
      // close the disconnected device in the callback
      SLIST_FOREACH_SAFE(cdc_dev, &p_cdc_acm_obj->cdc_devices_list, list_entry,
                         tcdc_dev) {
        if (cdc_dev->dev_hdl == event_msg->dev_gone.dev_hdl &&
            cdc_dev->notif.cb) {
          // The suddenly disconnected device was opened by this driver: inform
          // user about this
...
esp_err_t cdc_acm_host_data_tx_blocking(cdc_acm_dev_hdl_t cdc_hdl,
  ...
  CDC_ACM_CHECK(cdc_dev->data.out_xfer,
                ESP_ERR_NOT_SUPPORTED);  // Device was opened as read-only.
  CDC_ACM_CHECK(data_len <= cdc_dev->data.out_xfer->data_buffer_size,
                ESP_ERR_INVALID_SIZE);

  // Take OUT mutex and fill the OUT transfer
  BaseType_t taken =
      xSemaphoreTake(cdc_dev->data.out_mux, pdMS_TO_TICKS(timeout_ms));
  if (taken != pdTRUE) {
  ...
  ESP_GOTO_ON_ERROR(usb_host_transfer_submit(cdc_dev->data.out_xfer), unblock,
                    TAG, );

  // Wait for OUT transfer completion
  taken = xSemaphoreTake((SemaphoreHandle_t)cdc_dev->data.out_xfer->context,
  ...
  ESP_GOTO_ON_FALSE(
      ...
      ESP_ERR_INVALID_RESPONSE, unblock, TAG, "Bulk OUT transfer error");
  ESP_GOTO_ON_FALSE(cdc_dev->data.out_xfer->actual_num_bytes == data_len,
                    ESP_ERR_INVALID_RESPONSE, unblock, TAG,
...
esp_err_t cdc_acm_host_line_coding_get(cdc_acm_dev_hdl_t cdc_hdl,
                                       cdc_acm_line_coding_t *line_coding) {
  CDC_ACM_CHECK(line_coding, ESP_ERR_INVALID_ARG);

  ESP_RETURN_ON_ERROR(
      send_cdc_request((cdc_dev_t *)cdc_hdl, true, USB_CDC_REQ_GET_LINE_CODING,
                       (uint8_t *)line_coding, sizeof(cdc_acm_line_coding_t),
                       0),
      TAG, );
  ESP_LOGD(TAG,
           "Line Get: Rate: %" PRIu32
           ", Stop bits: %d, Parity: %d, Databits: %d",
           line_coding->dwDTERate, line_coding->bCharFormat,
           line_coding->bParityType, line_coding->bDataBits);
  return ESP_OK;
}

esp_err_t cdc_acm_host_line_coding_set(
    cdc_acm_dev_hdl_t cdc_hdl, const cdc_acm_line_coding_t *line_coding) {
  CDC_ACM_CHECK(line_coding, ESP_ERR_INVALID_ARG);

  ESP_RETURN_ON_ERROR(
      send_cdc_request((cdc_dev_t *)cdc_hdl, false, USB_CDC_REQ_SET_LINE_CODING,
                       (uint8_t *)line_coding, sizeof(cdc_acm_line_coding_t),
                       0),
      TAG, );
  ESP_LOGD(TAG,
           "Line Set: Rate: %" PRIu32
           ", Stop bits: %d, Parity: %d, Databits: %d",
           line_coding->dwDTERate, line_coding->bCharFormat,
           line_coding->bParityType, line_coding->bDataBits);
  return ESP_OK;
...
esp_err_t cdc_acm_host_set_control_line_state(cdc_acm_dev_hdl_t cdc_hdl,
                                              bool dtr, bool rts) {
  const uint16_t ctrl_bitmap = (uint16_t)dtr | ((uint16_t)rts << 1);

  ESP_RETURN_ON_ERROR(send_cdc_request((cdc_dev_t *)cdc_hdl, false,
                                       USB_CDC_REQ_SET_CONTROL_LINE_STATE, NULL,
                                       0, ctrl_bitmap),
                      TAG, );
...
esp_err_t cdc_acm_host_send_break(cdc_acm_dev_hdl_t cdc_hdl,
                                  uint16_t duration_ms) {
  ESP_RETURN_ON_ERROR(
      send_cdc_request((cdc_dev_t *)cdc_hdl, false, USB_CDC_REQ_SEND_BREAK,
                       NULL, 0, duration_ms),
      TAG, );
...
esp_err_t cdc_acm_host_send_custom_request(cdc_acm_dev_hdl_t cdc_hdl,
  ...
  CDC_ACM_CHECK(cdc_dev->ctrl_transfer->data_buffer_size >= wLength,
                ESP_ERR_INVALID_SIZE);
  ...
  usb_setup_packet_t *req =
      (usb_setup_packet_t *)(cdc_dev->ctrl_transfer->data_buffer);
  ...
  ESP_GOTO_ON_ERROR(
      usb_host_transfer_submit_control(p_cdc_acm_obj->cdc_acm_client_hdl,
                                       cdc_dev->ctrl_transfer),
      unblock, TAG, "CTRL transfer failed");

  taken = xSemaphoreTake(
      (SemaphoreHandle_t)cdc_dev->ctrl_transfer->context,
      pdMS_TO_TICKS(5000));  // This is a fixed timeout. Every CDC device should
                             // be able to respond to CTRL transfer in 5 seconds
  if (!taken) {
  ...
  ESP_GOTO_ON_FALSE(
      cdc_dev->ctrl_transfer->status == USB_TRANSFER_STATUS_COMPLETED,
      ESP_ERR_INVALID_RESPONSE, unblock, TAG, "Control transfer error");
  ESP_GOTO_ON_FALSE(cdc_dev->ctrl_transfer->actual_num_bytes ==
                        cdc_dev->ctrl_transfer->num_bytes,
                    ESP_ERR_INVALID_RESPONSE, unblock, TAG,
                    "Incorrect number of bytes transferred");
...
static esp_err_t send_cdc_request(cdc_dev_t *cdc_dev, bool in_transfer,
  ...
  return cdc_acm_host_send_custom_request(
      (cdc_acm_dev_hdl_t)cdc_dev, req_type, request, value,
      cdc_dev->notif.intf_desc->bInterfaceNumber, data_len, data);
}

esp_err_t cdc_acm_host_protocols_get(cdc_acm_dev_hdl_t cdc_hdl,
                                     cdc_comm_protocol_t *comm,
                                     cdc_data_protocol_t *data) {
  CDC_ACM_CHECK(cdc_hdl, ESP_ERR_INVALID_ARG);
  cdc_dev_t *cdc_dev = (cdc_dev_t *)cdc_hdl;
