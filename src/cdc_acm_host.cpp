 ...
 * Porting C++ done by Luc LEBOSSE 2024-02-12
 */
...
static esp_err_t cdc_acm_find_and_open_usb_device(uint16_t vid, uint16_t pid,
  ...
  *dev = (cdc_dev_t *)calloc(1, sizeof(cdc_dev_t));
  if (*dev == NULL) {
...
esp_err_t cdc_acm_host_install(
  ...
  esp_err_t ret;
  cdc_acm_obj_t *cdc_acm_obj = (cdc_acm_obj_t *)heap_caps_calloc(
      1, sizeof(cdc_acm_obj_t), MALLOC_CAP_DEFAULT);
  ...
  const usb_host_client_config_t client_config = {/* is_synchronous */
                                                  false,
                                                  /* max_num_event_msg */
                                                  3,
                                                  /* async */
  ...
  if (!err) {
    if (usb_host_client_register(&client_config, &usb_client) != ESP_OK) {
      ...
      // Between 1st call of this function and following section, another task
      // might try to install this driver: Make sure that there is only one
      // instance of this driver in the system
      CDC_ACM_ENTER_CRITICAL();
      if (p_cdc_acm_obj) {
        // Already created
  ...
  if (client_err) {
    if (usb_client) usb_host_client_deregister(usb_client);
  }
  if (err) {  // Clean-up
    free(cdc_acm_obj);
    if (event_group) {
...
static esp_err_t cdc_acm_transfers_allocate(cdc_dev_t *cdc_dev,
  ...
  ESP_GOTO_ON_ERROR(usb_host_transfer_alloc(USB_EP_DESC_GET_MPS(in_ep_desc), 0,
...
static esp_err_t cdc_acm_find_intf_and_ep_desc(cdc_dev_t *cdc_dev,
  ...
  if (interface_found) {
    ...
    do {
      ...
      cdc_dev->cdc_intf_desc = (const usb_standard_desc_t **)realloc(
...
esp_err_t cdc_acm_host_open(uint16_t vid, uint16_t pid, uint8_t interface_idx,
  ...
  if (ESP_OK != cdc_acm_find_and_open_usb_device(
    ...
    if (ESP_OK != cdc_acm_find_intf_and_ep_desc(cdc_dev, interface_idx,
      ...
      assert(cdc_dev->notif.intf_desc->bInterfaceSubClass ==
      ...
      if (ESP_OK != cdc_acm_transfers_allocate(cdc_dev, notif_ep, in_ep, out_ep,
                                               dev_config->out_buffer_size)) {
        err = true;
        ESP_LOGE(TAG, "cdc_acm_transfers_allocate");
      } else {
        if (ESP_OK != cdc_acm_start(cdc_dev, dev_config->event_cb,
                                    dev_config->data_cb,
                                    dev_config->user_arg)) {
          err = true;
          ESP_LOGE(TAG, "cdc_acm_start");
        } else {
...
esp_err_t cdc_acm_host_open_vendor_specific(
  ...
  if (ESP_OK != ret) {
    exit = true;
    ESP_LOGE(TAG, "cdc_acm_find_and_open_usb_device");
  } else {
    ...
    if (!cdc_dev->data.intf_desc) {
      ...
      err = true;
      ESP_LOGE(TAG, "Required interfece no %d was not found.", interface_num);
    } else {
      ...
      // Go through all interface's endpoints and parse Interrupt and Bulk
      // endpoints
      for (int i = 0; i < cdc_dev->data.intf_desc->bNumEndpoints; i++) {
        const usb_ep_desc_t *this_ep = usb_parse_endpoint_descriptor_by_index(
            cdc_dev->data.intf_desc, i, config_desc->wTotalLength,
            &desc_offset);
        assert(this_ep);

        if (USB_EP_DESC_GET_XFERTYPE(this_ep) == USB_TRANSFER_TYPE_INTR) {
          // Notification channel does not have its dedicated interface (data
          // and notif interface is the same)
          cdc_dev->notif.intf_desc = cdc_dev->data.intf_desc;
          notif_ep = this_ep;
        } else if (USB_EP_DESC_GET_XFERTYPE(this_ep) ==
      ...
      if (ESP_OK != cdc_acm_transfers_allocate(cdc_dev, notif_ep, in_ep, out_ep,
        ...
        // Allocate USB transfers, claim CDC interfaces and return CDC-ACM
        // handle
        if (ESP_OK != cdc_acm_start(cdc_dev, dev_config->event_cb,
                                    dev_config->data_cb,
                                    dev_config->user_arg)) {
...
esp_err_t cdc_acm_host_close(cdc_acm_dev_hdl_t cdc_hdl) {
  ...
  ESP_ERROR_CHECK(
      cdc_acm_reset_transfer_endpoint(cdc_dev->dev_hdl, cdc_dev->data.in_xfer));
  if (cdc_dev->notif.intf_desc != NULL) {
    ESP_ERROR_CHECK(
    ...
    default:
      ...
      if (cdc_dev->notif.cb) {
        const cdc_acm_host_dev_event_data_t error_event = {
            CDC_ACM_HOST_ERROR, (int)transfer->status};
...
static void in_xfer_cb(usb_transfer_t *transfer) {
  ...
  if (cdc_acm_is_transfer_completed(transfer)) {
    if (cdc_dev->data.in_cb) {
      cdc_dev->data.in_cb(transfer->data_buffer, transfer->actual_num_bytes,
                          cdc_dev->cb_arg);
    }
  ...
  if (cdc_acm_is_transfer_completed(transfer)) {
    ...
    switch (notif->bNotificationCode) {
      case USB_CDC_NOTIF_NETWORK_CONNECTION: {
        if (cdc_dev->notif.cb) {
          const cdc_acm_host_dev_event_data_t net_conn_event = {
              CDC_ACM_HOST_NETWORK_CONNECTION, (bool)notif->wValue};
      ...
      case USB_CDC_NOTIF_SERIAL_STATE: {
        ...
        if (cdc_dev->notif.cb) {
          cdc_acm_host_dev_event_data_t serial_state_event = {
